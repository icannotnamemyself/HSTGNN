...
class TNModule(nn.Module):
    def __init__(
        ...
        elif graph_build_type == "attsim_direc_tt_mask":
...
class AttentionSimDirectConstructor(nn.Module):
    ...
    def build_sub_graph(self, x1, x2, relation_name):
        ...
        adj_ = self.relation_attention_model[relation_name](x1, x2)  # [B, N*O, 1]
        # mask = torch.relu(torch.einsum("bnf, bmf -> bnm", x1, x2)) # self.relation_mask_model[relation_name](x_combined)  # [B, N*O, 1]
        
        # adj = adj_ * mask.reshape(B, N1, N2) # * adj_# [B, N, O]
    ...
    def build_st_graph(self, x1, x2):
        x1d = x1 - x1[:, :, -1:] 
        x2d = x2 - x2[:, -1:, :] 
        
        adj = torch.einsum("bnf, bmf -> bnm", x1, x2) + self.relation_attention_model['st'](x1d, x2d)
        return adj 

    def build_ts_graph(self, x2, x1):
        x1d = x1 - x1[:, :, -1:] 
        x2d = x2 - x2[:, -1:, :] 
        
        adj = torch.einsum("bnf, bmf -> bnm", x2, x1) + self.relation_attention_model['ts'](x2d, x1d)
        return adj 
    

    def uni_adj(self, adj):
        epsilon = 1e-12  # 一个小常数，防止数值不稳定
    ...
    def build_graph(self, spatial_nodes, temporal_nodes):
        ...
        st_adj  = self.build_st_graph(spatial_nodes, temporal_nodes )
        ts_adj = self.build_ts_graph(temporal_nodes, spatial_nodes)
        tt_adj = adj[:, N:, N:]
