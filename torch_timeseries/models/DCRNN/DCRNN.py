...
class DCRNNEncoder(BaseModel):
    def __init__(self, input_dim, adj_mat, max_diffusion_step, hid_dim, num_nodes,
                 num_rnn_layers, filter_type, device="cuda:0"):
...
class DCGRUDecoder(BaseModel):
    def __init__(self, input_dim, adj_mat, max_diffusion_step, num_nodes,
                 hid_dim, output_dim, num_rnn_layers, filter_type,device='cuda:0'):
        ...
        cell_with_projection = DCGRUCell(input_dim=hid_dim, num_units=hid_dim,
                                         adj_mat=adj_mat, max_diffusion_step=max_diffusion_step,
                                         num_nodes=num_nodes, num_proj=output_dim, filter_type=filter_type, device=self.device)
...
class DCRNNModel(BaseModel):
    def __init__(self, adj_mat, enc_input_dim, dec_input_dim, max_diffusion_step, num_nodes,
                 num_rnn_layers, rnn_units, seq_len, output_dim, filter_type, device="cuda:0"):
        ...
        self.encoder = DCRNNEncoder(input_dim=enc_input_dim, adj_mat=adj_mat,
                                    ...
                                    num_rnn_layers=num_rnn_layers, filter_type=filter_type, device=device)
        self.decoder = DCGRUDecoder(input_dim=dec_input_dim,
                                    adj_mat=adj_mat, max_diffusion_step=max_diffusion_step,
                                    num_nodes=num_nodes, hid_dim=rnn_units,
                                    output_dim=self._output_dim,
                                    num_rnn_layers=num_rnn_layers, filter_type=filter_type, device=self.device)
        assert self.encoder.hid_dim == self.decoder.hid_dim, \
            "Hidden dimensions of encoder and decoder must be equal!"

    def forward(self, source, target,batch_size, teacher_forcing_ratio=0.5):
        # (B, T, N, D)
        # the size of source/target would be (64, 12, 207, 2)
        # specify a GO symbol as the start of the decoder
        self.GO_Symbol = torch.zeros(1, batch_size, self._num_nodes * self._output_dim, 1).to(self.device)
