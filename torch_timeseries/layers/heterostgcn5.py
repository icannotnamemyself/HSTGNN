...
class HeteroFASTGCN(nn.Module):
    def __init__(
        ...
        self.bi_eps_parameter = nn.Parameter(torch.randn(1))
        self.eps_parameter = nn.Parameter(torch.randn(1))
        ...
        for i in range(self.num_layers):
            ...
            for bi in range(x.shape[0]):
                ...
                if i % 2 == 0: # intra
                    ...
                    xi = self.eps_parameter*x[bi] + torch.concat([out_dict['s'], out_dict['t']], dim=0)    
                    # print("prop intra")
                    ...
                    out_dict = self.convs[i](x_dict,edge_index_dict )
                    xi = self.bi_eps_parameter*x[bi] + out_dict['s'] + out_dict['t']
