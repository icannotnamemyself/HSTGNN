...
class WeightedHANConv(MessagePassing):
    ...
    def forward(
        ...
        for edge_type, edge_index in edge_index_dict.items():
            ...
            alpha_src = (x_src * lin_src)
            alpha_dst = (x_dst * lin_dst)
    ...
    def message(self, x_j: Tensor,x_i, alpha_i: Tensor, alpha_j: Tensor,
        ...
        alpha = self.edge_att_dict[edge_type](torch.concat((alpha_j,alpha_i,edge_weight), dim=-1))
...
#     def init_conv(self, in_channels, out_channels, **kwargs):
#         hstgnnlayer = WeightedHANConv(in_channels, out_channels,self.metadata,heads=self.heads, negative_slope=self.negative_slope,dropout=self.dropout)
#         return hstgnnlayer
...
#             intra_relation_ss = list()
#             intra_relation_tt = list()
#             intra_relation_st = list()
#             intra_relation_ts = list()
#             inter_relation_s = list()
#             inter_relation_t = list()
#             for bi in range(x.shape[0]):
#                 x_dict = {
#                     "s": x[bi][: self.node_num, :],
#                     "t": x[bi][self.node_num :, :],
...
#                 out_dict,intra_relation_dict,out_att = self.convs[i](x_dict, edge_index_dict,edge_weight_dict)
                
#                 if self.conv_type == 'ss':
#                     xi = torch.concat([out_dict["s"], x[bi][self.node_num: , :]], dim=0)
#                     intra_relation_ss.append(intra_relation_dict['ss'])
#                     inter_relation_s.append(out_dict['s'])
#                 elif self.conv_type == 'tt':
#                     xi = torch.concat([x[bi][:self.node_num , :] , out_dict["t"]], dim=0)
#                     intra_relation_tt.append(intra_relation_dict['tt'])
#                     inter_relation_t.append(out_dict['t'])
#                 else:
#                     xi = torch.concat([out_dict["s"], out_dict["t"]], dim=0)
#                     intra_relation_ss.append(intra_relation_dict['ss'])
#                     intra_relation_tt.append(intra_relation_dict['tt'])
#                     intra_relation_st.append(intra_relation_dict['st'])
#                     intra_relation_ts.append(intra_relation_dict['ts'])
#                     inter_relation_t.append(out_dict['t'])
#                     inter_relation_s.append(out_dict['s'])
...
#             x = torch.stack(xs)
#             nss = torch.stack(intra_relation_ss)
#             ntt = torch.stack(intra_relation_tt)
#             nst = torch.stack(intra_relation_st)
#             nts = torch.stack(intra_relation_ts)
#             smt_t = torch.stack(inter_relation_t)
#             smt_s = torch.stack(inter_relation_s)
...
#     ):
#         H, D = self.heads, self.out_channels // self.heads
#         x_node_dict, out_dict, intra_relation_dict = {}, {}, {}

#         # Iterate over node types:
#         for node_type, x in x_dict.items():
#             x_node_dict[node_type] = x.view(-1, H, D)# self.proj[node_type](x).view(-1, H, D)
#             out_dict[node_type] = []

#         # Iterate over edge types:
#         for edge_type, edge_index in edge_index_dict.items():
#             src_type, _, dst_type = edge_type
#             # intra_relation_dict[f'{src_type}{dst_type}'] = []
...
#             out_dict[dst_type].append(out)
#             intra_relation_dict[f'{src_type}{dst_type}'] = out
...
#         return out_dict ,intra_relation_dict, semantic_attn_dict
