...
class HEAT(nn.Module):
    ...
    def init_conv(self, in_channels, out_channels, **kwargs):
        ...
        # return FastHGTConv(in_channels, out_channels,self.metadata,heads=self.heads)
        return HEATConv(in_channels, out_channels,num_node_types=2,num_edge_types=2, edge_type_emb_dim=16, edge_dim=1, edge_attr_emb_dim=8, heads=self.heads)
...
# #         concat_x = torch.concat(xs, dim=-1)
# #         att_score = F.softmax(lin(concat_x), dim=-1) # 137 x 2
# #         # 137 * c , 137 * c
        
# #         for i in range(att_score.size(1)):
# #             out = att_score[i] * 
...
#         self.semantic_att_lin = nn.ModuleDict({
#             's': nn.Sequential(
...
#             x_att = torch.stack(outs)
#             s_att_in = torch.concat(outs, dim=-1)
#             att_scores = self.semantic_att_lin[node_type](s_att_in)
#             # import pdb;pdb.set_trace()
#             out = torch.einsum('kij, ik -> ij', x_att, att_scores)
#             out_dict[node_type] = out
#             semantic_attn_dict[node_type] = att_scores
