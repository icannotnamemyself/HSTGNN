...
class TCNOuputLayer(nn.Module):
    def __init__(self,input_seq_len,num_nodes,out_seq_len,tcn_layers,dilated_factor,in_channel,tcn_channel,end_channel) -> None:
        super().__init__()
        ...
        self.tcn = TCN(
                    ...
                    dilated_factor=dilated_factor,end_channel=end_channel
                )
        self.end_layer = nn.Conv2d(end_channel, out_seq_len, (1, 1))
        self.act = nn.ELU()
...
class TCN(nn.Module):
    def __init__(
        ...
        dilated_factor=2, dropout=0,kernel_set=[2,3,6,7],end_channel=128,
    ):
        super().__init__()
        ...
        if dilated_factor>1:
            self.receptive_field = int(1+(max_kernel_size-1)*(dilated_factor**num_layers-1)/(dilated_factor-1))
        else:
            self.receptive_field = num_layers*(max_kernel_size-1) + 1
        ...
        for i in range(1):
            if dilated_factor>1:
                ...
                rf_size_i = i*num_layers*(max_kernel_size-1)+1
            new_dilation = 1
            
            for j in range(1,num_layers+1):
                if dilated_factor > 1:
                    rf_size_j = int(rf_size_i + (max_kernel_size-1)*(dilated_factor**j-1)/(dilated_factor-1))
                else:
