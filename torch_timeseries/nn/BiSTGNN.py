...
class TNTCN(nn.Module):
    def __init__(self,input_seq_len,n_nodes, act='gelu', without_gc=False,one_node_forecast=False, on_time=True, on_space=True, tcn_channel=16, pred_horizon=3, multi_pred=False,dilated_factor=2, tcn_layers=4, 
                 remain_prob=0.2, casting_dim=16, gc_layers=3, edge_mode='', aggr_mode='', dropout=0.05, gcn_eps=0.05, gcn_channel=16, gcn_type='', no_time=False, no_space=False):
        super().__init__()
        self.act = activation_resolver.make(act)

        self.without_gc = without_gc
        if not without_gc:
            self.tn_module = TNModule(input_seq_len, n_nodes, remain_prob, act, casting_dim, gc_layers, edge_mode, aggr_mode, dropout, gcn_eps, gcn_channel, gcn_type, no_time, no_space)
...
class TNMLP(nn.Module):
    def __init__(self, without_gc, act, n_nodes, input_seq_len, num_classes) -> None:
        super().__init__()

        self.without_gc = without_gc
        if not self.without_gc:
            self.tn_module = TNModule(input_seq_len, n_nodes, remain_prob, act=act, casting_dim, gc_layers, edge_mode, aggr_mode, dropout=0.1, gcn_eps=0.01, gcn_channel, gcn_type, no_time, no_space)
        self.act = activation_resolver.make(act)
...
class TNModule(nn.Module):
    def __init__(self, input_seq_len, n_nodes, remain_prob, act, casting_dim, gc_layers, edge_mode, aggr_mode, dropout, gcn_eps, gcn_channel,gcn_type, no_time, no_space ):
        super().__init__()
...
class MyGCN(nn.Module):
    def __init__(
        ...
        act='relu', act_first=False, batch_adj=False
    ):
        super().__init__()
        ...
        self.convs.append(
            self.init_conv(in_channels, hidden_channels,batch_adj))
        for _ in range(n_layers - 2):
            self.convs.append(
                self.init_conv(hidden_channels, hidden_channels,batch_adj))
        if out_channels is not None:
            self.convs.append(
                self.init_conv(hidden_channels, out_channels,batch_adj))
        else:
            self.convs.append(
                self.init_conv(hidden_channels, hidden_channels,batch_adj))
    ...
    def init_conv(self, in_channels, out_channels, batch_adj):
        return GraphConvolution(in_channels, out_channels, batch_adj)
