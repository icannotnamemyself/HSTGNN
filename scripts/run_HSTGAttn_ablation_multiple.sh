...
# ./scripts/run_HSTGAttn_ablation_multiple.sh "3 6 12" "METR_LA PEMS_BAY" "cuda:0" 12 
# ./scripts/run_HSTGAttn_ablation_multiple.sh "3 6 12" "PEMS04 PEMS07" "cuda:1" 12 
# ./scripts/run_HSTGAttn_ablation_multiple.sh "12" "METR_LA PEMS_BAY" "cuda:0" 12 
...
do
    ...
    do
        ...
        CUDA_DEVICE_ORDER=PCI_BUS_ID python3 ./torch_timeseries/cli/hstgnnv3.py  --invtrans_loss=True --dataset_type="$dataset" --device="$device" --normalization=False --batch_size=32 --without_gcn=True  --horizon=1 --pred_len="$pred_len" --windows $windows --epochs=100 config_wandb --project=BiSTGNN --name="HSTGAttn_ablation" runs --seeds='[42,233,666,19971203,19980224]'
    done
done
