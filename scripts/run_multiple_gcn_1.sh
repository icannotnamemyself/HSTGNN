...
do
    ...
    do
        ...
        CUDA_DEVICE_ORDER=PCI_BUS_ID python3 ./torch_timeseries/cli/hstgnnv7.py  --invtrans_loss=True --dataset_type="$dataset" --latent_dim=64 --gcn_layers=1 --device="$device" --batch_size=32 --horizon=1 --pred_len="$pred_len" --windows $windows --epochs=100 config_wandb --project=BiSTGNN --name="HSTGNNv7" runs --seeds='[42,233,666]'
    done
done
